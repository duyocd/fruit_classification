...
for root, dirs, files in os.walk(data_dir):
    for filename in files:
        if filename.lower().endswith(('.jpg', '.png', '.jpeg')):
            ...
            if folder_name in ['apple', 'avocado', 'banana', 'cherry', 'kiwi', 'mango', 'orange', 'pinenapple', 'strawberries', 'watermelon']:
...
X = np.array([preprocess_input(img_to_array(load_img(img, target_size=(224, 224)))) for img in image_paths])

# Chia tập dữ liệu thành tập huấn luyện và tập kiểm tra
X_train, X_test, y_train, y_test = train_test_split(X, y_numeric, train_size=0.8, test_size=0.2, random_state=42)
